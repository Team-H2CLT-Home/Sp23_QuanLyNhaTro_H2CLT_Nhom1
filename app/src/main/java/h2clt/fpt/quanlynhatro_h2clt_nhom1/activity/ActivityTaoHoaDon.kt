...
//        list = HoaDonDao(this).getAllInHoaDonByMaKhu(maKhu)
        ...
        val dichVuDien = listDV.find { it.ten_loai_dich_vu=="Tiền điện"}
        val dichVuNuoc= listDV.find { it.ten_loai_dich_vu=="Tiền nước" }
                ...
                if(dichVuDien!=null && dichVuNuoc !=null) {
                    ...
                    when {
                        dichVuDien != null && dichVuDien.so_cu < 0 -> {
                            dichVuDien.ma_loai_dich_vu.let { it1 ->
                                loaiDichVuPhongDao.updateLoaiDichVu(
                                    it1,
                                    soDienCu,
                                    soDienMoi
                                )
                    ...
                    when {
                        dichVuNuoc != null && dichVuNuoc.so_cu < 0 && dichVuNuoc.trang_thai_loai_dich_vu == 1 -> {
                            loaiDichVuPhongDao.updateLoaiDichVu(
                                dichVuNuoc.ma_loai_dich_vu,
                                soNuocCu,
                                soNuocMoi
                            )
                        }
